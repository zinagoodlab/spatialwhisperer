...
from spatialwhisperer.jointemb.conch_text_processor import ConchTextProcessor
import lightning as pl

from transformers import AutoTokenizer
from spatialwhisperer.jointemb.processing import TranscriptomeTextDualEncoderProcessor

from spatialwhisperer.config import (
    config as cellwhisperer_config,
    get_path,
...
class JointEmbedDataModule(pl.LightningDataModule):
    ...
    def prepare_data(self, force_prepare=False):
        ...
        logger.debug(f"[TIMING] prepare_data START, datasets: {self.dataset_names}")
        for dataset_name in self.dataset_names:
            logger.debug(f"[TIMING] Preparing dataset: {dataset_name}")
            self.prepare_dataset(dataset_name, force_prepare)
            logger.debug(f"[TIMING] Dataset {dataset_name} prepared.")

    def prepare_dataset(self, dataset_name, force_prepare):
        logger.debug(f"[TIMING] prepare_dataset: {dataset_name}")
        sample_ids = self.get_sample_ids(dataset_name)
        logger.debug(f"[TIMING] sample_ids for {dataset_name}: {sample_ids}")
                ...
                if (
                    ...
                    if False:  # TODO disable for time reasons
                        ...
                        if not individual_samples_dir.exists():
                            ...
                            try:
                                result = torch.load(
                                    str(processed_path), mmap=True, weights_only=False
                                )
                                orig_ids = result[0][
                                    "orig_ids"
    ...
    def prepare_dataset_file(self, adata_path, processed_path, dataset_name, sample_id):
        ...
        logger.debug(
            f"[TIMING] prepare_dataset_file START: {dataset_name}/{sample_id}, adata_path={adata_path}"
        )

        # Load data
        logger.debug(f"[TIMING] Reading h5ad from {adata_path} ...")
        adata = anndata.read_h5ad(adata_path)
        logger.debug(
            f"[TIMING] h5ad loaded: {adata.shape} in {_time.perf_counter() - _t0:.1f}s"
        )
        ...
        logger.debug(f"[TIMING] Starting processor (transcriptome+image) ...")
        ...
        logger.debug(
            f"[TIMING] processor done in {_time.perf_counter() - _t0:.1f}s, input keys: {list(inputs.keys())}, shapes: { {k: v.shape for k, v in inputs.items() if hasattr(v, 'shape')} }"
        )
        ...
        logger.debug(
            f"[TIMING] Filtering done in {_time.perf_counter() - _t0:.1f}s, {sum(valid_datapoints_filter)}/{len(valid_datapoints_filter)} valid cells"
        )

        # process/generate the individual files (only needed for disk-based loading)
        if self.use_disk_loading:
            logger.debug(f"[TIMING] Starting individual file generation ...")
            self._generate_individual_sample_files(inputs, dataset_name, sample_id)
            logger.debug(
                f"[TIMING] Individual files done in {_time.perf_counter() - _t0:.1f}s"
            )
        ...
        # When disk loading is used, only orig_ids is needed from this file — skip large patch tensors
        if self.use_disk_loading:
            inputs_to_save = {"orig_ids": inputs["orig_ids"]}
        else:
            inputs_to_save = inputs

        logger.info(
            f"Saving processed dataset to {processed_path} (keys: {list(inputs_to_save.keys())})"
        )
        torch.save(
            (inputs_to_save, replicate_inputs),
            processed_path,
        )
        logger.info(
            f"Saved processed dataset ({processed_path.stat().st_size / 1e9:.1f} GB)"
        )
    ...
    def _load_disk_datasets(self, dataset_name):
        ...
        for sample_id in sample_ids:
            ...
            result = torch.load(str(processed_path), mmap=True, weights_only=False)
