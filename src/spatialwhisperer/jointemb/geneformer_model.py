...
# Load gene symbol → ensembl id mapping. Preference order:
#   1) `resources/ensembl_gene_symbol_map.csv` (legacy cellwhisperer path,
#      kept for backward compat with checkouts that have the file in-place)
#   2) The copy vendored next to this module (always present in a fresh clone)
_legacy_path = get_path(["paths", "ensembl_gene_symbol_map"])
_vendored_path = Path(__file__).resolve().parent.parent / "data" / "ensembl_gene_symbol_map.csv"

if _legacy_path.exists():
    ANNOT = pd.read_csv(_legacy_path, index_col=0)
elif _vendored_path.exists():
    ANNOT = pd.read_csv(_vendored_path, index_col=0)
else:
    # No cached file → fetch from biomart and write to the vendored location
    # (use_cache=False avoids `sqlite3.OperationalError: database is locked`)
    annot = sc.queries.biomart_annotations(
        "hsapiens", ["ensembl_gene_id", "external_gene_name"], use_cache=False
    ).set_index("external_gene_name")
    annot_drop_dups = (
        annot.reset_index()
        .drop_duplicates(subset="external_gene_name")
        .set_index("external_gene_name")
    )
    _vendored_path.parent.mkdir(parents=True, exist_ok=True)
    annot_drop_dups.to_csv(_vendored_path)
