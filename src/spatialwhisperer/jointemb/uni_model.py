...
from spatialwhisperer.config import config
        ...
        **kwargs: Any,
        ...
        if isinstance(image, openslide.OpenSlide):
            ...
            try:
                ...
                image_np = np.array(
                    image.read_region((0, 0), 0, (img_w, img_h)).convert("RGB")
                )
            except Exception:
                logger.warning(
                    "Could not convert OpenSlide to numpy; falling back to sequential extraction"
                )
                image_np = None
        ...
        if image_np is not None:
            ...
            loader = DataLoader(
                patch_ds, batch_size=256, shuffle=False, num_workers=n_workers
            )
            X_context = []
            X_cell = []
