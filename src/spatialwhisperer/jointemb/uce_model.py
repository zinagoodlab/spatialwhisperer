...
Model checkpoints:
  - KuanP/uce-cosmx-geneset (original, no assay token)
  - KuanP/cosmx_cxg_lbcl_gene_subset_assay_token (with assay token support)
...
from data_collection_suite.data.cell_sentence_sampler import (
    sample_cell_sentences_mapping_gene,
)

from spatialwhisperer.config import config as cellwhisperer_config, get_path

TARGET_SUM = 1e4

# Ensembl ID -> gene symbol fallback (some hest1k samples have only Ensembl IDs
# in var.gene_name; UCE's vocab is keyed by symbol so without translation those
# samples produce 0 overlap and crash sample_cell_sentences_mapping_gene).
# Reuse the same mapping file the Geneformer processor builds.
_ENSEMBL_TO_SYMBOL: Optional[Dict[str, str]] = None


def _load_ensembl_to_symbol() -> Dict[str, str]:
    global _ENSEMBL_TO_SYMBOL
    if _ENSEMBL_TO_SYMBOL is None:
        path = get_path(["paths", "ensembl_gene_symbol_map"])
        annot = pd.read_csv(path, index_col=0)  # index: symbol, col: ensembl_gene_id
        # Invert: ensembl_id -> symbol; drop empty + duplicate ensembl_ids.
        inv = (
            annot.reset_index()
            .dropna(subset=["ensembl_gene_id", "external_gene_name"])
            .drop_duplicates(subset="ensembl_gene_id")
            .set_index("ensembl_gene_id")["external_gene_name"]
        )
        _ENSEMBL_TO_SYMBOL = inv.to_dict()
    return _ENSEMBL_TO_SYMBOL

# Assay token indices for the assay-token-aware UCE checkpoint
# (KuanP/cosmx_cxg_lbcl_gene_subset_assay_token).
# These use existing positions in the protein embedding vocabulary (vocab_size=145469).
# Set assay_token_idx=None to disable (backward compat with old checkpoints).
...
class _CellDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        species: str = "human",
        assay_token_idx: Optional[int] = None,
    ):
        self.adata = adata
        ...
        self.assay_token_idx = assay_token_idx
    ...
    def _align_genes(self, species: str):
        ...
        # Some hest1k h5ads carry Ensembl IDs in gene_name (no symbol annotation).
        # UCE's vocab is keyed by gene symbol, so translate Ensembl -> symbol here.
        if h5ad_gene_names and h5ad_gene_names[0].startswith("ENSG"):
            e2s = _load_ensembl_to_symbol()
            # strip Ensembl version suffix (e.g. ENSG00000243485.1 -> ENSG00000243485)
            stripped = [g.split(".")[0] for g in h5ad_gene_names]
            translated = [e2s.get(g, g) for g in stripped]
            n_translated = sum(1 for o, t in zip(stripped, translated) if o != t)
            logging.info(
                f"UCE Ensembl->symbol translation: {n_translated}/{len(h5ad_gene_names)} "
                f"genes mapped from Ensembl IDs to symbols"
            )
            h5ad_gene_names = translated
    ...
    def __getitem__(self, idx):
        ...
        result = sample_cell_sentences_mapping_gene(
            ...
            seed=idx,
            assay_token_idx=self.assay_token_idx,
        )
...
def _load_gene_data(
    gene_names_path: str, gene_mapping_path: str, species: str = "human"
):
...
class UCETranscriptomeProcessor(ProcessorMixin):
    ...
    When config.yaml uce_paths.use_assay_token is true, inserts an assay token
    after CLS based on adata.uns.get('is_cosmx', False):
      - True  -> COSMX_ASSAY_TOKEN_IDX (145461)
      - False -> NON_COSMX_ASSAY_TOKEN_IDX (145460)
    When false, no assay token is inserted (backward compat with old checkpoints).
    """
    ...
    def __call__(self, adata, *args, **kwargs) -> dict:
        ...
        if cellwhisperer_config["uce_paths"]["use_assay_token"]:
            is_cosmx = adata.uns.get("is_cosmx", False)
            assay_token_idx = (
                COSMX_ASSAY_TOKEN_IDX if is_cosmx else NON_COSMX_ASSAY_TOKEN_IDX
            )
            logging.info(
                f"UCE assay token: is_cosmx={is_cosmx} -> assay_token_idx={assay_token_idx}"
            )
        else:
            assay_token_idx = None
...
class UCEModel(PreTrainedModel):
    ...
    def from_pretrained(
        ...
        # Load Kuan's model from HuggingFace Hub (checkpoint set in config.yaml).
        # Read raw config value (not get_path, which prepends PROJECT_ROOT).
        checkpoint = cellwhisperer_config["uce_paths"]["checkpoint"]
