...
Aggregation convention for macro AUROC and related macro metrics:
1. Compute one-vs-rest per-class metrics separately within each dataset/seed.
2. Average those per-class metrics across classes to obtain a per-dataset macro score.
3. Average the per-dataset macro scores across datasets (and across seeds where applicable).

Importantly, we do NOT pool all samples across datasets into one global AUROC per class.
"""
...
for fp in adata_fps:
    ...
    # Filter background class (consistent with prediction notebook filter_background=True)
    bg_indices = [i for i, c in enumerate(classes) if c.lower() == "background"]
    if bg_indices:
        keep = [i for i in range(len(classes)) if i not in bg_indices]
        classes = [classes[i] for i in keep]
        counts_df = counts_df.iloc[:, keep]
    obs_labels = counts_df.columns[counts_df.values.argmax(axis=1)].values.astype(str)
...
for fp in score_fps:
    ...
    try:
        ...
        assert len(sdf.columns) == len(classes), (
            "score CSV column count must match number of classes"
        )
    ...
    # Macro metrics are computed within each dataset first (mean across classes),
    # and only later averaged across datasets. This avoids a pooled global AUROC
    # over all samples from all datasets.
    macro = {
        ...
        "soft_rocauc_macroAvg": float(
            np.nanmean(per_class_df_this["soft_rocauc"].values)
        ),
        "recall_at_5_macroAvg": float(
            np.nanmean(per_class_df_this["recall_at_5"].values)
        ),
    }
...
if not per_dataset_df.empty:
    per_dataset_mean = per_dataset_df.groupby("dataset", as_index=False).mean(
        numeric_only=True
    )
else:
...
if per_class_rows_all:
    ...
    per_class_mean = per_class_all.groupby("class_label", as_index=False).mean(
        numeric_only=True
    )
    # Per-class by dataset (seed-averaged) for error bars in per-class plots
    per_class_by_dataset = per_class_all.groupby(
        ["dataset", "class_label"], as_index=False
...
aggregated = {
    k: float(per_dataset_mean[k].mean())
    if (k in per_dataset_mean.columns and len(per_dataset_mean))
    else float("nan")
    for k in AGG_KEYS
}
