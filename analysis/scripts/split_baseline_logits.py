...
def parse_dataset_id(x: str, prediction_level: str) -> str:
    # CRC datasets follow the regNNN_[AB] convention.
    m = re.search(r"(reg\d+_[AB])", x)
    if m:
        return m.group(1)
    # Generic fallback (Lizard, PanNuke, ...): strip the _<level>.tiff suffix.
    return re.sub(rf"_{re.escape(prediction_level)}\.tiff$", "", x)

if "dataset_id" not in df.columns:
    # derive from source_image if present
    if "source_image" in df.columns:
        df["dataset_id"] = df["source_image"].apply(
            lambda v: parse_dataset_id(v, prediction_level)
        )
    else:
