...
rule subset_performance_trend_grid_pannuke:
    """
    Variant of subset_performance_trend_grid that uses the in-house PanNuke
    tissue-type benchmark (19-organ zero-shot patch classification) for the
    I->T column instead of the MUSK skin benchmark.

    Reads tissue_type_all_summary.csv (produced by compute_tissue_type_metrics
    in disease_stratification.smk) for each I->T subsampling combo.
    """
    input:
    ...
    output:
        plot=BENCHMARKS_DIR / "subset_performance" / "trend_grid_pannuke.svg"
    params:
    ...
    script:
        "../scripts/plot_subset_trends_pannuke.py"
